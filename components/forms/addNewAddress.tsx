...
export default function AddNewAddressForm() {
  ...
  const operationalAddressObject = myArray.find(
    (item) => item.operationalAddress
  );
  const operationalAddress =
    operationalAddressObject && operationalAddressObject.operationalAddress;
  const operationalAddressCenter = operationalAddress
    ? { lat: operationalAddress.latitude, lng: operationalAddress.longitude }
    : center;
            ...
            (item, index) =>
              item.oilContainer && (
                <div key={`oilContainer-${index}`} className={styles.fakeInput}>
                  {item.oilContainer.locationName}, {item.oilContainer.lat},{" "}
                  {item.oilContainer.lng}
                  <div className={styles.inputIconButton}>
                    <button type="button">
                      <LiaEdit
                        size={26}
            ...
            (item, index) =>
              item.greaseTrap && (
                <div key={`greaseTrap-${index}`} className={styles.fakeInput}>
                  {item.greaseTrap.locationName}, {item.greaseTrap.lat},{" "}
                  {item.greaseTrap.lng}
                  <div className={styles.inputIconButton}>
                    <button type="button">
                      <LiaEdit
                    ...
                    <button
                      type="button"
                      onClick={() => removeField("greaseTrap", index)}
                    >
                      <FiTrash
                        size={22}
                        style={{ color: "rgba(76, 142, 59, 1)" }}
